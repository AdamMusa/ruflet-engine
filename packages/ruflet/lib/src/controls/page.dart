...
class _PageControlState extends State<PageControl> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    _routeInformationProvider = isEmbedded
        ? RufletLocalRouteInformationProvider(initialRouteInformation: initial)
        : RufletRouteInformationProvider(initialRouteInformation: initial);
    if (!isEmbedded) {
      RufletDeepLinkingBootstrap.markRouterReady();
    }
  ...
  void didChangeDependencies() {
    ...
    _ensureServiceRegistries();
    _loadFontsIfNeeded(RufletBackend.of(context));
  }
  ...
  void _routeChanged() {
    RufletBackend.of(context).onRouteUpdated(_routeState.route);
  }
  ...
  Future<void> _loadFontsIfNeeded(RufletBackend backend) async {
  ...
  Widget build(BuildContext context) {
    ...
    if (!widget.control.backend.multiView) {
      ...
      var appStatus = context
          .select<RufletBackend, ({bool isLoading, String error})>((backend) =>
  ...
  Widget _buildApp(Control control, Widget? home) {
    ...
    // theme
    var themeMode = control.getRufletThemeMode("theme_mode") ??
    ...
    var brightness = context.select<RufletBackend, Brightness>(
  ...
  Widget _buildNavigator(
    ...
    var appStatus =
        context.select<RufletBackend, ({bool isLoading, String error})>(
